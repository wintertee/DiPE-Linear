...
class FITS(nn.Module):
    ...
    def __init__(
        ...
        self.dominance_freq = (self.seq_len // base_T + 1) * h_order + 10
            ...
            for i in range(self.channels):
                self.freq_upsampler.append(
                    nn.Linear(self.dominance_freq,
                              int(self.dominance_freq * self.length_ratio)).to(
                                  torch.cfloat))
        ...
        if self.individual:
            low_specxy_ = torch.zeros([
                ...
                low_specx.size(2)
                                      ...
                                      dtype=low_specx.dtype).to(
                                          low_specx.device)
            for i in range(self.channels):
                low_specxy_[:, :, i] = self.freq_upsampler[i](
                    low_specx[:, :, i].permute(0, 1)).permute(0, 1)
        else:
            low_specxy_ = self.freq_upsampler(low_specx.permute(0, 2,
                                                                1)).permute(
                                                                    0, 2, 1)
        # print(low_specxy_)
